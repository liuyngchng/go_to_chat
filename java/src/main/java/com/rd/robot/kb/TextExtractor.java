...
public class TextExtractor {
    ...
    private static String extractPDF(String filePath) throws Exception {
        try (PDDocument document = Loader.loadPDF(new File(filePath))) {
            ...
            if (numPages == 0) {
                throw new RuntimeException("PDF 文件为空");
            }
            ...
            if (result.isEmpty()) {
                throw new RuntimeException("PDF 文件无文本内容，可能是扫描件");
            }
            return result;
        }
    }
    ...
    private static String extractDOCX(String filePath) throws Exception {
        try (FileInputStream fis = new FileInputStream(filePath);
            ...
            if (result.isEmpty()) {
                throw new RuntimeException("DOCX 文件无文本内容");
            }
            return result;
        }
    }
    ...
    private static String extractXLSX(String filePath) throws Exception {
        ...
        try (FileInputStream fis = new FileInputStream(filePath);
            ...
            String result = sb.toString().trim();
            if (result.isEmpty()) {
                throw new RuntimeException("XLSX 文件无内容");
            }
            return result;
        }
    }
